...
class BluetoothDevice {
  ...
  Future<List<BluetoothService>> discoverServices() async {
    Future<List<BluetoothService>> fut = _convertStreamToFuture(
        FlutterBlue.instance._servicesDiscoveredChannel
            ...
            .map((p) => p.services)
            .map((s) =>
                s.map((p) => new BluetoothService.fromProto(p)).toList()),
        "discoverServices");
  ...
  Future<List<int>> readCharacteristic(
    ...
    FlutterBlue.instance._log(LogLevel.info,
        'remoteId: ${id.toString()} characteristicId: ${characteristic.id} serviceUuid: ${characteristic.serviceUuid.toString()}');

    await FlutterBlue.instance._channel
        .invokeMethod('readCharacteristic', request.writeToBuffer());

    return _convertStreamToFuture(
        FlutterBlue.instance._characteristicReadChannel
            .receiveBroadcastStream()
            .map((buffer) =>
  ...
  Future<List<int>> readDescriptor(BluetoothDescriptor descriptor) async {
    var request = protos.ReadDescriptorRequest.create()
      ...
      ..characteristicId = descriptor.characteristicId.toProto
    ...
    return _convertStreamToFuture(
        FlutterBlue.instance._descriptorReadChannel
            .receiveBroadcastStream()
            .map((buffer) =>
  ...
  Future<Null> writeCharacteristic(
    ...
    var fut = _convertStreamToFuture(
  ...
  Future<Null> writeDescriptor(
    ...
    var fut = _convertStreamToFuture(
        FlutterBlue.instance._methodStream
            ...
            .map((m) => m.arguments)
            .map((buffer) =>
                new protos.WriteDescriptorResponse.fromBuffer(buffer))
            .where((p) =>
                (p.request.remoteId == request.remoteId) &&
                (p.request.descriptorUuid == request.descriptorUuid) &&
                (p.request.characteristicUuid == request.characteristicUuid) &&
  ...
  Stream<List<int>> onValueChanged(BluetoothCharacteristic characteristic) {
    return FlutterBlue.instance._methodStream
        ...
        .where((c) => c.id == characteristic.id)
        .map((c) {
      characteristic.updateDescriptors(c.descriptors);
      characteristic.value = c.value;
      return c.value;
